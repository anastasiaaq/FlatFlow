...
| Auth | **Session + HTTP-only cookies** | Not readable by JS (XSS-safe); trivial logout; CSRF built into Django | JWT in localStorage — XSS-exposed, needs blacklist for logout |
