...
export function listIssues() {
  return request<{ data: IssueDetail[]; status: 200 }>('/api/issues/')
}

export function createIssue(title: string, description: string) {
  return request<{ data: IssueDetail; status: 201 }>('/api/issues/', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export function updateIssue(
  issue: Pick<IssueDetail, 'id' | 'title' | 'description' | 'status'>,
) {
  return request<{ data: IssueDetail; status: 200 }>(`/api/issues/${issue.id}/`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      title: issue.title,
...
export function deleteIssue(id: number) {
  return request<{ data: void; status: 204 }>(`/api/issues/${id}/`, {
    method: 'DELETE',
  })
}
